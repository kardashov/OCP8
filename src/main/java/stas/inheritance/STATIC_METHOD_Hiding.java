...
public class STATIC_METHOD_Hiding {
	...
	public static void eat() {System.out.println("StaticMethodHiding is eating");	}

	public static void sneeze() {System.out.println("StaticMethodHiding is sneezing");}

	public void hibernate() {System.out.println("StaticMethodHiding is hibernating");	}
	
	
	public static void main(String[] args) {
	//—татические методы должны вызыватьс€ по имени класса:
		
		Panda.eat(); //Panda bear is chewing
		STATIC_METHOD_Hiding.eat(); //StaticMethodHiding is eating
		
	//≈сли все-таки есть вызов по ссылке, то
	//статические методы ¬—≈√ƒј –≈«ќЋ¬я“—я ѕќ “»ѕ” ——џЋ » ¬ќ ¬–≈ћя  ќћѕ»Ћя÷»».
	//ќбъект какого типа  находитс€ в переменной во врем€ выполнени€ никого не волнует(даже null):
		Panda p = new Panda();
		...
		smh = p;   	
		smh.eat();  //StaticMethodHiding is eating, несмотр€ на то, что ссылка уже содержит объект типа Panda
...
//	class. The compiler detects that youТre trying to override a method that should be hidden
...
//	thinks that youТre trying to hide a method that should be overridden and also generates a
//	compiler error.

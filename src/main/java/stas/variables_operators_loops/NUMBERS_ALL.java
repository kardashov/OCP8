...
public class NUMBERS_ALL {
	...
	public static void main(String[] s) {
		...
		ss = finalInt;  // а так работает, т.к. finalInteger  -- известна€ заранее константа  
		final int finalBigInt = 1533443434;
		//ss = finalBigInt; // не компилируетс€, т.к. константа слишком больша€
		
		// ss = bb + ss; ошибка компил€ции. нельз€ вписать int в short
		ss += bb; // однако это работает без проблем
		ss += Integer.MAX_VALUE; // и даже это работает
		ss++; // и это работает
		...
		//
		System.out.println(-Integer.MIN_VALUE); // выводит все равно -2147483648
												// !!!

		byte x = -128;
		System.out.println(-x); // 128 сначала идет преобразование к int,
								// а потом оператор "-" !

		byte y = 127;
		System.out.println(++y); // -128 декремент/инкремент не делают
									// преобразование к int, поэтому здесь
									// переполнение

		double ccc = 2147483647; // может быть integer-литерал вплоть до
									// Integer.MAX_VALUE
		// ccc = 2147483648; // а это уже не работает
...
//		values, and then cast the result to an int.
		...
		float f = a;
		a = (long) f;
		System.out.println(a);// 111111110656, в float не вмещаютс€ все значащие
								// цифры из long
		a = (long) 3455.34234;
		System.out.println(a); //3455 дробна€ часть double обрезаетс€
...
//		want to remove the 2, you can write numbers.remove(new Integer(2)) to force wrapper class use.
		...
		numbers.add(1);
		numbers.add(2);
		numbers.remove(1); //удаление элемента с индексом 1, т.к. автобокс не срабатывает!  numbers.remove(new Integer(2)) -- удал€ет элемент по значению

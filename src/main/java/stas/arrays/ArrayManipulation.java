...
//		numbers22 = { 42, 55, 99, }; // DOES NOT COMPILE  Array constants can only be used in initializers
		...
		int[] numbers345 = { 42,
							 55, 	// shortcut с зап€той после последнего элемента.
							 34, }; // фича €зыка дл€ удобства редактировани€ кода
		
		System.out.println("================== ќѕ»–ќ¬јЌ»≈ —ќƒ≈–∆»ћќ√ќ ћј——»¬ј===================================");
//		System.arraycopy(src, srcPos, dest, destPos, length); -- копирует содержимое одного массива в другой
		...
		for (String string : strings)  //enhanced FOR LOOP doesn't accept Map
			System.out.print(string + " "); // 10 100 9 -- сортировка строк в
											// алфавитном пор€дке
		...
		// Take note of the fact that line 3 is a sorted array. If it werenТt,
		// we couldnТt apply either
		// of the other rules. Line 4 searches for the index of 2. The answer is
		// index 0. Line 5 searches
		// for the index of 4, which is 1.
		// Line 5 searches for the index of 1. Although 1 isnТt in the list, the
		...
		// answer of Ц1. Line 7
		// is similar. Although 3 isnТt in the list, it would need to be
		// inserted at element 1 to preserve
		// the sorted order. We negate and subtract 1 for consistency, getting
		// Ц1 Ц1, also known as
		// Ц2. Finally, line 8 wants to tell us that 9 should be inserted at
		// index 4. We again negate and
		// subtract 1, getting Ц4 Ц1, also known as Ц5.
		...
		String[][] rectangle = new String[3][2]; // пр€моугольный массив
		rectangle[0][1] = "set";

		int[][] differentSize = { { 1, 4 }, { 3 }, { 9, 8, 7 } }; // массив
																	// произвольной
																	// формы

		// Another way to create an asymmetric array is to initialize just an
		// arrayТs first dimension,
...
//		 birds1.add(8, "blue jay"); // исключение index out of bound exception
...
//		 System.out.println(birds2.remove(0)); // исключение index out of bound exception 
...
//			page 135!!!
			...
			heights.add(null);
//			int h = heights.get(0); // NullPointerException, т.к. autoboxing пытаетс€ вызвать метод parseInt() на null-ссылке
			Integer h = heights.get(0); // но это работает без проблем, т.к. autoboxing не срабатывает
			
			
//			It actually outputs 1. After adding the two values, the List contains [1, 2]. We then request
//			the element with index 1 be removed. ThatТs right: index 1. Because thereТs already a remove()
...
//			want to remove the 2, you can write numbers.remove(new Integer(2)) to force wrapper class use.
			...
			numbers222.add(1);
			numbers222.add(2);
			numbers222.remove(1); //удаление элемента с индексом 1, т.к. автобокс не срабатывает!  
								  //	numbers.remove(new Integer(2)) -- удал€ет элемент по значению

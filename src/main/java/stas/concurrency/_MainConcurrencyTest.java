...
/*	====void execute(Runnable command) 
	...
	====<T> T invokeAny(Collection<? extends Callable<T>> tasks) throws InterruptedException, ExecutionException 
		...
		Future<?> futureResult1 = service.submit(() -> {System.out.println("sss");} );//Runnable version
		...
		try {System.out.println(futureResult.get()); //prints 50
		...
		for(Integer i:list){
			...
			taskList.add(() -> { System.out.println(i); return i;});// adds task objects to Collection
		}
...
//		result.cancel(boolean mayInterruptIfRunning);//Attempts to cancel execution of the task.
		...
		try {
			...
			i = result.get(1, TimeUnit.SECONDS);}//waits for result some time then throws TimeoutException
		...
		if (service != null) {
		   try {service.awaitTermination(1, TimeUnit.MINUTES);// Check whether all tasks are finished
